...
def verify_deployment_manifest(
    *,
    ...
    """Validate evaluator provenance and semantic contracts before motion."""
    ...
    if not all(
        isinstance(item, dict) for item in (checkpoint, evaluation, data, contract)
    ):
    ...
    if checkpoint.get("selection_reason") != "lowest safe validation deployment-prior L1":
